...
class StampVisualIdsTest extends TestCase
{
  ...
  public function test_three_level_bard_replicator_bard_nesting_all_get_uuids(): void
  {
    ...
    $outerValues = $data['body'][0]['attrs']['values'];
    $this->assertNotEmpty($outerValues['_visual_id'], 'outer Bard set should have _visual_id');

    // Level 2: inner Replicator set nested inside the outer Bard set
    $innerRepSet = $outerValues['inner_rep'][0];
    $this->assertNotEmpty($innerRepSet['_visual_id'], 'inner Replicator set should have _visual_id');

    // Level 3: innermost Bard set nested inside the inner Replicator set
    $innermostValues = $innerRepSet['inner_bard'][0]['attrs']['values'];
    $this->assertNotEmpty($innermostValues['_visual_id'], 'innermost Bard set should have _visual_id');
  }
  ...
  public function test_plain_scalar_fields_adjacent_to_replicator_are_not_modified(): void
  {
    ...
    $this->assertArrayNotHasKey('_visual_id', $data);
    $this->assertNotEmpty($data['items'][0]['_visual_id']);
  }
}
